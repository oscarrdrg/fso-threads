...
int n_fil,
n_col;
/* numero de files i columnes del taulell */
int m_por;
/* mida de la porteria (en caracters) */
int f_pal,
c_pal;
/* posicio del primer caracter de la paleta */
int m_pal;
/* mida de la paleta */
int f_pil,c_pil; 
/* posicio de la pilota, en valor enter */
float pos_f,pos_c;
/* posicio de la pilota, en valor real */
float vel_f,vel_c;
/* velocitat de la pilota, en valor real */
int nblocs=0;
int dirPaleta=0;
int retard;
/* valor del retard de moviment, en mil.lisegons */
int fi1=0, fi2=0;

char strin[LONGMISS];

int numPilotes = 0;
/* variable per a generar missatges de text */
...
int carrega_configuracio(FILE * fit) {
	...
	if ((n_fil !=0) || (n_col !=0)) {
		/* si no dimensions maximes */
		if ((n_fil < MIN_FIL) || (n_fil > MAX_FIL) || (n_col < MIN_COL) || (n_col > MAX_COL)) ret=1;
		else if (m_por > n_col - 3) ret=2;
		else if ((m_pal > n_col - 3) || (m_pal < 1) || (f_pal > n_fil-1) || (f_pal < 1) || (c_pal + m_pal > n_col -1 || c_pal < 1)) ret=3;
		else if ((pos_f< 1) || (pos_f >=n_fil - 3) || (pos_c < 1) || (pos_c > n_col - 1))
		/* tres files especials: línia d'estat, porteria i paleta */
		ret=4;
	}

	if ((vel_f< -1.0) || (vel_f > 1.0) || (vel_c < -1.0) || (vel_c > 1.0)) ret=5;

	if (ret !=0) {
		/* si ha detectat algun error */
...
int inicialitza_joc(void) {
	...
	/* limita valor de la porteria */
	if (m_por==0) m_por=3 * (n_col - 2) / 4;
	/* valor porteria per defecte */

	i_port=n_col / 2 - m_por / 2 - 1;
	/* crea el forat de la porteria */
	f_port=i_port+m_por - 1;
	for (i=i_port; i <=f_port; i++) win_escricar(n_fil - 2, i, ' ', NO_INV);

	n_fil=n_fil - 1;
	/* descompta la fila de missatges */

	for (i=0; i < m_pal; i++)
	/* dibuixar paleta inicial */
	win_escricar(f_pal, c_pal + i, '0', INVERS);

	/* generar la pilota */
	if (pos_f > n_fil - 1) pos_f=n_fil - 1;
	/* limita posicio inicial de la pilota */
...
void control_impacte(void) {
	if (dirPaleta==TEC_DRETA) {
		if (vel_c <=0.0)
		/* pilota cap a l'esquerra */
		vel_c = -vel_c - 0.2;

		/* xoc: canvi de sentit i reduir velocitat */
		else {
			/* a favor: incrementar velocitat */
			if (vel_c <=0.8) vel_c+=0.2;
		}
	}
		...
		if (dirPaleta==TEC_ESQUER) {
			if (vel_c>=0.0)
			/* pilota cap a la dreta */
			vel_c = -vel_c+0.2;

			/* xoc: canvi de sentit i reduir la velocitat */
			else {
				/* a favor: incrementar velocitat */
				if (vel_c >=-0.8) vel_c -=0.2;
			}
			...
			if (dirPaleta==TEC_AMUNT) vel_c=0.0;

			/* vertical */
			else {
				if (dirPaleta==TEC_AVALL) if (vel_f <=1.0) vel_f -=0.2;
				/* frenar */
			}
		}
	}
	...
	dirPaleta=0;
	/* reset perque ja hem aplicat l'efecte */
}
...
void * mou_pilota(void * index) {
	...
	do {
		...
		f_h=pos_f+vel_f;
		/* posicio hipotetica de la pilota (entera) */
		c_h=pos_c+vel_c;
		rh=rv=rd=' ';
		...
		if ((f_h !=f_pil) || (c_h !=c_pil)) {
			...
			if (f_h !=f_pil) {
				...
				/* veure si hi ha algun obstacle */
				if (rv !=' ') {
					/* si hi ha alguna cosa */
					comprovar_bloc(f_h, c_pil);
					if (rv=='0')
					/* col.lisió amb la paleta? */
					//					control_impacte();
					vel_c=control_impacte2(c_pil, vel_c);
					vel_f=-vel_f;
					/* canvia sentit velocitat vertical */
					f_h=pos_f+vel_f;
					/* actualitza posicio hipotetica */
				}
			}
			...
			if ((f_h !=f_pil) && (c_h !=c_pil)) {
				...
				if (rd !=' ') {
					...
					vel_f=-vel_f;
					vel_c=-vel_c;
					/* canvia sentit velocitats */
					f_h=pos_f+vel_f;
					c_h=pos_c+vel_c;
					/* actualitza posicio entera */
				}
			}
...
void * mou_paleta(void * null) {
	...
	do {
		...
		if (tecla !=0) {
			...
			if (tecla==TEC_RETURN) fi1=1;
			/* final per pulsacio RETURN */
			dirPaleta=tecla;
			/* per a afectar al moviment de les pilotes */
		}
...
int main(int n_args, char *ll_args[]) {
	...
	if ((n_args !=2) && (n_args !=3)) {
		/* si numero d'arguments incorrecte */
		i=0;
		do fprintf(stderr, "%s", descripcio[i++]);
		/* imprimeix descripcio */
	...
	exit(3);
	/* aborta si hi ha algun problema en el fitxer */

	if (n_args==3) {
		/* si s'ha especificat parametre de retard */
	...
	/* altrament, fixar retard per defecte */

	printf("Joc del Mur: prem RETURN per continuar:\n");
	getchar();
	if (inicialitza_joc() !=0)
	/* intenta crear el taulell de joc */
	exit(4);
	/* aborta si hi ha algun problema amb taulell */


	/* DECLARAMOS LAS VARIABLES DEL THREAD */
	pthread_t threadPilota, threadPaleta;
	
	if(pthread_create(&threadPaleta, NULL, mou_paleta, NULL)==0)
		fprintf(stderr,"he creado la paleta");
	
	
	if(pthread_create(&threadPilota, NULL, mou_pilota, &numPilotes)==0)
		fprintf(stderr,"he creado la pelota");
	
	numPilotes++;

	int temps = 0, segons = 0, minuts = 0;
	while(fi1 == 0 && fi2 == 0){
		...
		sprintf(strin,"%im:%is", minuts, segons);
		win_escristr(strin);
	}
	pthread_join(threadPilota, NULL);
	pthread_join(threadPaleta, NULL);

	printf("HAN ACABADO TODOS LOS THREADS");

	if (nblocs==0){
		char watch[LONGMISS];
		sprintf(watch,"YOU WIN! - %im:%is", minuts, segons);
		mostra_final(watch);
	}else{
		char watch[LONGMISS];
		sprintf(watch,"GAME OVER - %im:%is", minuts, segons);
		mostra_final(watch);
	} 
	...
	return (0);
	/* retorna sense errors d'execucio */
}